...
net_eurusd = fitnet(hiddenLayerSize,trainFcn);
...
net_eurusd.trainParam.epochs = 60;
% Setup Division of Data for Training, Validation, Testing
% For a list of all data division functions type: help nndivision
net_eurusd.divideFcn = 'dividerand';  % Divide data randomly
net_eurusd.divideMode = 'sample';  % Divide up every sample
...
% Train the Network
[net_eurusd,tr] = train(net_eurusd,x,t);
...
if (false)
    ...
    genFunction(net_eurusd,'myNeuralNetworkFunction');
    y = myNeuralNetworkFunction(x);
end
if (false)
    % Generate a matrix-only MATLAB function for neural net_eurusdwork code
    % generation with MATLAB Coder tools.
    genFunction(net_eurusd,'myNeuralNetworkFunction','MatrixOnly','yes');
    y = myNeuralNetworkFunction(x);
end
if (false)
